% Calculate ASI regimes from correlations between SST and turbulent heat
% flux
% Alex Kinsella, February 2026
% Updated to use 1985-2020
...
%% 2: Calculate lagged corr for sliding months
nlag = 30;
...
%save('/Volumes/proj/mahadevanlab/kinsella/output/asi_regimes/manuscript/sst_thf_anomaly_corrs_260209.mat','sst_thf_anomaly_corrs','-v7.3')
...
%100*sum(p_SST_naive(:)<0.05)/sum(~isnan(p_SST_naive(:)))
%100*sum(p_SST_eff(:)<0.05)/sum(~isnan(p_SST_eff(:)))
...
lags = -30:30;
c = nan([size(squeeze(sst_thf_anomaly_corrs.corrs_tend(:,:,1,:))),2]);
resnorm = nan(size(squeeze(sst_thf_anomaly_corrs.corrs_tend(:,:,1,:))));
residual = nan([size(squeeze(sst_thf_anomaly_corrs.corrs_tend(:,:,1,:))),2*numel(lags)]);

for dd = 1:size(sst_thf_anomaly_corrs.corrs_tend,4)
    for ii = 1:size(sst_thf_anomaly_corrs.corrs_tend,1)
        for jj = 1:size(sst_thf_anomaly_corrs.corrs_tend,2)

            SST = squeeze(sst_thf_anomaly_corrs.corrs_SST(ii,jj,:,dd));
            tend = squeeze(sst_thf_anomaly_corrs.corrs_tend(ii,jj,:,dd));
...
asi_regimes.lon = flux_turb_NIO.lon;
asi_regimes.lat = flux_turb_NIO.lat;
asi_regimes.param = c(:,:,:,1).^2./(c(:,:,:,1).^2+c(:,:,:,2).^2);
%save('/Volumes/proj/mahadevanlab/kinsella/output/asi_regimes/manuscript/asi_regimes_weighted_OAFlux_260210.mat','asi_regimes','-v7.3')
